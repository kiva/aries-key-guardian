...
export class SmsOtpDbGateway {
    ...
    public async fetchSmsOtp(agentId: string): Promise<SmsOtp> {
        const smsOtp: SmsOtp | undefined = await this.smsOtpRepository.findOne({agentId});
        if (!smsOtp) {
    ...
    public async savePhoneNumber(agentId: string, phoneNumber: string): Promise<SmsOtp> {
        ...
        return this.smsOtpRepository.manager.transaction(async (entityManager: EntityManager) => {
            let smsOtp: SmsOtp | undefined = await entityManager.findOne(SmsOtp, {agentId});
            if (!smsOtp) {
                smsOtp = new SmsOtp();
                smsOtp.agentId = agentId;
            }
